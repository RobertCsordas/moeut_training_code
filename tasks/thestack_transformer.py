...
def a(parser: framework.helpers.ArgumentParser):
    ...
    parser.add_argument("-thestack.languages", default="python,html,c++,rust,javascript,haskell,scala,assembly")


@task()
class ThestackTransformer(LMEvalMixin, LMBase):
    ...
    def create_datasets(self):
        ...
        self.train_set = dataset.TheStack(self.helper.args.thestack.languages, self.helper.args.lm.unroll, split="train", n_tokens=self.helper.args.sentencepiece.n_pieces, token_limit=train_token_limit)
        self.valid_sets.val = dataset.TheStack(self.helper.args.thestack.languages, self.helper.args.lm.unroll, split="validation", n_tokens=self.helper.args.sentencepiece.n_pieces, token_limit=test_token_limit)
